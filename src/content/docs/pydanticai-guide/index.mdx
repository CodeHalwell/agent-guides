...
## What's new in v1.94.0 (May 2026)

- **v1.94.0 (May 12)** — Minor release; new top-level exports added: `AgentRun`, `AgentRunResult`, `StreamedRunResultSync` (synchronous counterpart to `StreamedRunResult` for CLI/notebook use). All existing guide symbols (`Agent`, `RunContext`, `ModelRetry`, `AgentRunResult`, `StreamedRunResult`, `UsageLimits`, `RunUsage`, `capture_run_messages`) confirmed present in installed `pydantic-ai==1.94.0` (`.routine-envs/check-0512-py`); no `DeprecationWarning` emissions.
